...
public class Action {
    ...
    private static void debilusActionHandler(PawnController[] playerArray, PawnController ctrl) {
        ...
        if (action == 0){ //Tries and moves.
            ...
            //TODO if (ctrl.getBoard().getCell(ctrl.getDependency().getCoord()).hasPawn()) { --> c'est pas là où est notre pawn ?
            if (ctrl.getBoard().getCell(forwardCell).hasPawn()) {
                ...
                if (!(Rules.canMove(ctrl, direction, forwardCell))) {
                    ...
                    //TODO Coord forwardForwardCell = Coord.add(forwardCell, direction); --> cell derrière le Pawn d'en face non ?
                    ...
                    if (choice == 0) {
                        ...
                        if (Rules.canMove(ctrl, directionBis, forwardCell)) { //TODO forwardForwardCell
                            ctrl.move(direction); //moves on the same cell as the other pawn
                            //TODO ctrl.move(direction); //moves on the cell behind the other pawn ?
